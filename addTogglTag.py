...
token = ""         
entryName = "Entry started by addTogglTag script"  # the default description for a new entry.
...
def parseArguments():
  ...
  parser = argparse.ArgumentParser(description='Add tags to running Toggl '
    'entries: will create a new one if none is running')
  parser.add_argument("-v", "--verbose", help="Increase output verbosity",
    action="store_true")
  parser.add_argument("-t", "--tag", help="The name of the tag to add", 
    required=True)
  parser.add_argument("-w", "--workspace", 
    help="The name of the workspace a new entry is started in; "
    "default is the first encountered")
  parser.add_argument("-n", "--entrydname", 
    help="The name used for an entry, if a new one is created; "
    "has no meaning when supplied with --dontcreate "
    "default is '"+entryName+"'")
  parser.add_argument("--dontcreate", 
    help="Don't create a new entry if none is running", action="store_true")
